...
struct LogsView: View {
    ...
    var filteredLogs: [BotManager.LogEntry] {
        botManager.logs.filter { entry in
            ...
            VStack(spacing: 16) {
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Real-time bot activity and output")
                            .font(.system(size: 14))
                ...
                HStack(spacing: 12) {
                    ...
                    HStack(spacing: 4) {
                        FilterButton(label: "All", isSelected: filterLevel == nil) {
                            filterLevel = nil
                        }
                        FilterButton(label: "Info", isSelected: filterLevel == .info, color: .blue) {
                            filterLevel = .info
                        }
                        FilterButton(label: "Warn", isSelected: filterLevel == .warning, color: .yellow) {
                            filterLevel = .warning
                        }
                        FilterButton(label: "Error", isSelected: filterLevel == .error, color: .red) {
                            filterLevel = .error
                        }
                    }
    ...
    var body: some View {
        Button(action: action) {
            Text(label)
                ...
                .background(
                    Capsule()
                        .fill(isSelected ? color.opacity(0.8) : .secondary.opacity(0.1))
                )
        }
        .buttonStyle(.plain)
    }
