...
export class TelegramOpencodeBot {
  constructor(config) {
    ...
    this.lmstudio = new LmStudioClient({
      lmsBin: config.lmsBin || undefined,
      baseURL: config.lmstudioBaseURL || undefined
    })
    this.state = null
  ...
  async handleUpdate(update) {
    const nextOffset = Number.isFinite(update.update_id) ? update.update_id + 1 : null
    ...
    try {
      ...
      await this.enqueueChat(chat.id, async () => {
        await this.processMessage(message, text)
      })
      shouldAcknowledge = true
    } finally {
      if (shouldAcknowledge && Number.isFinite(nextOffset)) {
        const currentOffset = Number.isFinite(this.state.offset) ? this.state.offset : null
  ...
  async processMessage(message, text) {
    ...
    try {
      ...
      // Ensure the active LM Studio model is loaded before running opencode.
      await this._ensureLmStudioModel(this.config.opencodeModel)
  ...
  async handleModelCommand(chatId, argsText) {
    ...
    if (!requested) {
      const loadedId = await this.lmstudio.getLoadedModelId().catch(() => null)
      await this.telegram.sendText(chatId, this.modelText(models, defaultModel, loadedId))
      return
    }
    ...
    if (requested === 'default' || requested === 'reset') {
      ...
      await this.stateStore.save(this.state)
      const loadStatus = await this._ensureLmStudioModel(defaultModel)
    ...
    // If the new model is an LM Studio model, ensure it is loaded before
    // telling the user everything is ready.
    const loadStatus = await this._ensureLmStudioModel(nextModel)
  ...
  modelText(models, defaultModel, loadedLmStudioId) {
    const activeModel = this.activeModel()
    const loadedLine = loadedLmStudioId
      ? `LM Studio loaded: ${loadedLmStudioId}`
      : 'LM Studio loaded: (none)'
    return [
      ...
      'Available models:',
      ...models.map((model) => {
        const localId = LmStudioClient.toLocalId(model)
        const tag = localId && localId === loadedLmStudioId ? ' [loaded]' : ''
        return `- ${model}${tag}`
      }),
      '',
      'Usage:',
  ...
  /**
   * If `opencodeModelName` is an LM Studio model, ensure it is loaded in
   * LM Studio (unloading any other model if necessary).
   *
   * Returns a human-readable status string (empty when not applicable).
   */
  async _ensureLmStudioModel(opencodeModelName) {
    if (!LmStudioClient.isLmStudioModel(opencodeModelName)) return ''

    const localId = LmStudioClient.toLocalId(opencodeModelName)
    if (!localId) return ''

    try {
      const { changed } = await this.lmstudio.ensureModelLoaded(localId)
      return changed
        ? `LM Studio: loaded "${localId}".`
        : ''
    } catch (error) {
      console.error('[lmstudio] ensureModelLoaded error:', error)
      return `LM Studio warning: ${error.message}`
    }
  }
